    ...
    async def request(self, flow: http.HTTPFlow) -> None:
        ...
        async with self.semaphore:
            proxy_info = await get_proxy(
                self.http_client, target_url=target_url, user_jwt=user_jwt
            )
            ...
            try:
                ...
                async with httpx.AsyncClient(
                    ...
                    try:
                        ...
                        logger.error(
                            f"Timeout while forwarding request to {target_url}"
                        )
                        flow.response = http.Response.make(
                            504, b"Gateway Timeout", {"Content-Type": "text/plain"}
                        )
                ...
                flow.response = http.Response.make(
                    500, b"Internal Server Error", {"Content-Type": "text/plain"}
                )
