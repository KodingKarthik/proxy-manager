...
def list_proxies(
    working_only: bool = Query(default=False, description="Filter to working proxies only"),
    limit: int = Query(default=100, ge=1, le=1000, description="Maximum number of proxies to return"),
    ...
    proxies = get_all_proxies(session, working_only=working_only, limit=limit, offset=offset)
    return [
        ProxyResponse(
            id=p.id,
            ip=p.ip,
...
def get_proxy_by_strategy(
    ...
    target_url: Optional[str] = Query(None, description="Target URL to check against blacklist"),
    current_user: User = Depends(get_current_user),
    session: Session = Depends(get_session)
):
    """
